...
from finetunings.generate_epochs.datasets import TokensBatcher
...
def test_shuffling(mock_load_fn):
    ...
    batcher = TokensBatcher(Path("some/path"), known_qids, batch_size)

    initial_index = batcher.data_index.copy()
    batcher.shuffler()
    shuffled_index = batcher.data_index
...
def test_remove_when_qid_missing(mock_load_fn):
    batcher = TokensBatcher(Path("some/path"), [], 2)
