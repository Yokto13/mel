...
def evaluate_reranking(
    damuel_token: str = "/lnet/work/home-students-external/farhan/troja/outputs/v2_normal_filtered/descs_pages",
    damuel_embs: str = "/lnet/work/home-students-external/farhan/troja/outputs/workdirs/asi_se_to_rozbilo_init_all/damuel_for_index_6",
    mewsli_tokens: str = "/lnet/work/home-students-external/farhan/troja/outputs/tokens_mewsli_finetuning",
    reranking_model_path: str = "/lnet/work/home-students-external/farhan/troja/outputs/workdirs/asi_se_to_rozbilo_init_all/models_5/ema.pth",
    reranker_checkpoint_path: str = "/lnet/work/home-students-external/farhan/troja/outputs/reranking_models/pairwise_mlp/70000.pth",
    ...
    #     "/lnet/work/home-students-external/farhan/troja/outputs/models/LEALLA-base",
    #     state_dict_path=reranking_model_path,
    #     mlp_hidden_dim=2048,
    # )
    ...
    # )
    reranker.load(
        # "/lnet/work/home-students-external/farhan/troja/outputs/reranking_models/full_lealla_r/750000.pth",
        "/lnet/work/home-students-external/farhan/troja/outputs/reranking_models/full_lealla_192/360000.pth"
    )
    reranker.eval()
    reranker.to(device)

    # Resolve tokens and embeddings (directories or files)
    damuel_tokens, damuel_token_qids = load_tokens_qids_from_dir(damuel_token, verbose=True)
    damuel_embs, damuel_qids = load_embs_and_qids(damuel_embs)
    ...
    mewsli_root = Path(mewsli_tokens)
    ...
    for language, mewsli_path in language_paths:
        ...
        mewsli_loader = DataLoader(mewsli_dataset, batch_size=batch_size, shuffle=False)
        ...
        for mewsli_tokens_batch, qids_batch in mewsli_loader:
            ...
            neighbor_qids = searcher.find(
                mewsli_embs.to("cpu").numpy().astype(np.float16), num_neighbors=num_neighbors
            )
    ...
    overall_accuracy = (
        round(sum(all_accuracies) / len(all_accuracies), 4) if all_accuracies else 0.0
    )
    print(all_accuracies)
    print(f"\nOverall accuracy across languages: {overall_accuracy}")
...
if __name__ == "__main__":
    # Preserve CLI behavior for direct execution
    ...
    parser.add_argument(
        "--reranker_checkpoint_path",
        type=str,
        default="/lnet/work/home-students-external/farhan/troja/outputs/reranking_models/pairwise_mlp/70000.pth",
        help="Path to trained reranker checkpoint (.pth)",
    )
    parser.add_argument(
        "--num_neighbors",
        type=int,
    ...
    evaluate_reranking(
        damuel_token=cli_args.damuel_token,
        damuel_embs=cli_args.damuel_embs,
        mewsli_tokens=cli_args.mewsli_tokens,
        reranking_model_path=cli_args.reranking_model_path,
        reranker_checkpoint_path=cli_args.reranker_checkpoint_path,
        num_neighbors=cli_args.num_neighbors,
