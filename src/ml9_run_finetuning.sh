...
STATE_DICT="$WORKDIR/models_9/final.pth"

if [ ! -e "$WORKDIR/models_10/final.pth" ]; then
    echo "Running round 10"

    ./run_ml_finetuning_round.sh "$DAMUEL_DESCS_TOKENS_RAW" "$DAMUEL_LINKS_TOKENS_RAW" "$MEWSLI_TOKENS_RAW" "$MODEL_PATH"\
     "$WORKDIR" "$BATCH_SIZE" "$EPOCHS" "$LOGIT_MULTIPLIER" "$LR" $STATE_DICT 10 "$TYPE" "$N_OF_ROUNDS" $NEG 8
fi
