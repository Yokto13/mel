...
class Director:
    def construct_descriptions_for_finetuning(self, damuel_path: str, tokenizing_params, n_of_extractors=None, current_extractor_n=None):
        ...
        builder.set_entry_processor(self._get_description_entry_processor(tokenizing_params))

    @classmethod
    def _get_tokenizer_wrapper(cls, tokenizing_params: TokenizingParams):
        assert cls._check_token_in_tokenizer(tokenizing_params.tokenizer, tokenizing_params.mention_token)
    ...
    @classmethod
    def _check_token_in_tokenizer(cls, tokenizer: PretrainedTokenizerBase, token: str) -> bool:
