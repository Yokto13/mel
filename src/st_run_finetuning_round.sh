...
if [ ! "$(ls -A $DAMUEL_FOR_INDEX_DIR)" ]; then
    ...
    sbatch --wait -p "gpu-troja,gpu-ms" -N1 -G 8 -C "gpuram24G|gpuram16G" --mem=150G run ../venv/bin/python $ACTION_SCRIPT "embs_from_tokens_model_name_and_state_dict" "$DAMUEL_DESC_TOKENS" "$MODEL_PATH" 50000 "$DAMUEL_FOR_INDEX_DIR" "$STATE_DICT"
fi
...
if [ ! "$(ls -A $DAMUEL_LINKS_DIR)" ]; then
    ...
    sbatch --wait -p "gpu-troja,gpu-ms" -N1 -G 8 -C "gpuram24G|gpuram16G" --mem=150G run ../venv/bin/python $ACTION_SCRIPT "embed_links_for_generation" "$DAMUEL_LINKS_TOKENS" "$MODEL_PATH" 50000 "$DAMUEL_LINKS_DIR" "$STATE_DICT"
fi
...
if [ ! "$(ls -A $DAMUEL_FOR_INDEX_NEW_DIR)" ]; then
    echo "Running embs generating for damuel"
    sbatch --wait -p "gpu-troja,gpu-ms" -N1 -G 8 -C "gpuram24G|gpuram16G" --mem=150G run ../venv/bin/python $ACTION_SCRIPT "embs_from_tokens_model_name_and_state_dict" "$DAMUEL_DESC_TOKENS" "$MODEL_PATH" 50000 "$DAMUEL_FOR_INDEX_NEW_DIR" "$MODELS_DIR/final.pth"
