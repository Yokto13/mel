...
MODEL_CONFIG_PATH="../configs/paraphrase_m.gin"
TRAIN_CONFIG_PATH="../configs/train_p.gin"

DAMUEL_DESCS_TOKENS_RAW="$OUTPUTS/v2_normal_p/descs_pages"
DAMUEL_LINKS_TOKENS_RAW="$OUTPUTS/v2_normal_p/links"
MEWSLI_TOKENS_RAW="$OUTPUTS/finetuning_mewsli2_p"
WORKDIR="$OUTPUTS/workdirs/train_p"
N_OF_ROUNDS=5
...
run_ml_finetuning_round() {
    ...
    local LINKS_PER_ROUND=$(($STEPS_PER_EPOCH * 250000 * 8))
    ...
    if [ ! "$(ls -A $DAMUEL_LINKS_TOKENS)" ]; then
        uv run python $ACTION_SCRIPT "copy" \
    ...
    if [ ! "$(ls -A $DAMUEL_FOR_INDEX_DIR)" ]; then
        ...
        uv run python $ACTION_SCRIPT "embs_from_tokens_model_name_and_state_dict" \
    ...
    if [ ! "$(ls -A $DAMUEL_LINKS_DIR)" ]; then
        ...
        uv run python $ACTION_SCRIPT "embed_links_for_generation" \
    ...
    if [ ! "$(ls -A $BATCH_DIR)" ]; then
        ...
        # uv run python -m cProfile -o "generate.prof" $ACTION_SCRIPT "generate" \
        uv run python $ACTION_SCRIPT "generate" \
    ...
    if [ ! "$(ls -A $MODELS_DIR)" ]; then
        ...
        #uv run python -m cProfile -o "train_ddp.prof" $ACTION_SCRIPT "train_ddp" \
        echo $ACTION_SCRIPT "train_ddp" \
            --DATASET_DIR="$BATCH_DIR" \
            --MODEL_SAVE_DIR="$MODELS_DIR" \
            --STATE_DICT_PATH="$STATE_DICT"
        uv run python $ACTION_SCRIPT "train_ddp" \
            --DATASET_DIR="$BATCH_DIR" \
            --MODEL_SAVE_DIR="$MODELS_DIR" \
            --STATE_DICT_PATH="$STATE_DICT"
    fi
    ...
    if [ ! "$(ls -A $DAMUEL_FOR_INDEX_NEW_DIR)" ]; then
        ...
        uv run python $ACTION_SCRIPT "embs_from_tokens_model_name_and_state_dict" \
            ...
            --state_dict_path="$MODELS_DIR/ema.pth"
    fi
    ...
    for LANG in "${LANGUAGES[@]}"; do
        ...
        # uv run python $ACTION_SCRIPT "recalls" \
    ...
    uv run python $ACTION_SCRIPT "evaluate" \
        --root_dir="$WORKDIR" \
        --finetuning_round=$ROUND_ID     
...
do
    ...
    STATE_DICT="$WORKDIR/models_$ROUND_ID/ema.pth"
done
