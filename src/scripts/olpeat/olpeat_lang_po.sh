...
MEWSLI_INPUT="$OUTPUTS/embs_mewsli_at_po/$LANG"
MEWSLI_INPUT_EMBS="$MEWSLI_INPUT/embs_qids.npz"

cd ../embs

INPUT="$OUTPUTS/tokens_damuel_at/$LANG"
sbatch --wait -p "gpu-troja,gpu-ms" -G 8 -C "gpuram16G" --mem=220G damuel_at_lang_v2.sh $INPUT "pooler_output" $DAMUEL_INPUT
...
sbatch --wait -p "gpu-troja,gpu-ms" -G 1 -C "gpuram16G" mewsli_at_lang.sh "$LANG" "pooler_output" $MEWSLI_INPUT
