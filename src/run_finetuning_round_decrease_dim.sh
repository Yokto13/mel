...
if [ ! "$(ls -A $MODELS_DIR)" ]; then
    ...
    echo $ACTION_SCRIPT "train" "$BATCH_DIR" "$MODEL_PATH" "$EPOCHS" "$LOGIT_MULTIPLIER" "$LR" "$TYPE" "$MODELS_DIR" "$STATE_DICT" "$TARGET_DIM"
    ...
    sbatch --wait -p "gpu-troja,gpu-ms" -G $CARDS -C "gpuram40G|gpuram24G" --mem=100G run ../venv/bin/python $ACTION_SCRIPT "train_ddp" "$BATCH_DIR" "$MODEL_PATH" "$EPOCHS" "$LOGIT_MULTIPLIER" "$LR" "$MODELS_DIR" "$STATE_DICT" "$TARGET_DIM"
fi
